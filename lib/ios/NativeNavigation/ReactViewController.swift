...
private let kAirbnbInstanceId = "airbnbInstanceId"
...
private func propsWithMetadata(_ props: [String: AnyObject], airbnbInstanceId: String) -> [String: AnyObject] {
  var newProps = props
  newProps[kAirbnbInstanceId] = airbnbInstanceId as AnyObject?
  return newProps
}
...
open class ReactViewController: UIViewController {
  ...
  public init(moduleName: String, props: [String: AnyObject] = [:]) {
    self.airbnbInstanceId = generateId(moduleName)
    self.moduleName = moduleName
    self.props = propsWithMetadata(props, airbnbInstanceId: airbnbInstanceId)
  ...
  deinit {
    coordinator.unregisterViewController(airbnbInstanceId)
  }
  ...
  fileprivate var hideStatusBarUntilFoldOffset: Bool = false
  ...
  fileprivate var snapToFoldOffset: Bool = false
  fileprivate var transition: ReactSharedElementTransition?


  private func updateNavigationImpl() {
    // TODO(lmr):
  }
  ...
  func updateNavigation() {
    ...
    if (isPendingNavigationTransition) {
      ...
      onTransitionCompleted = self.updateNavigationImpl
    } else {
  ...
  func signalFirstRenderComplete() {
    // do nothing. on iOS, the navigation update call seems to come after the first render call... so we want to continue using
    // that as the best signal for when to actually execute the pending navigation. This may change in the future, but it's working
...
extension UIViewController {
  public func presentReactViewController(_ viewControllerToPresent: ReactViewController, animated: Bool, completion: (() -> Void)?) {
    ...
    let realPresent = { [weak self, weak viewControllerToPresent] in
      ...
      let identifier = viewControllerToPresent.airbnbInstanceId
