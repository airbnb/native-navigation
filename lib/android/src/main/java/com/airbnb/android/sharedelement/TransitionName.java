...
/**
 * Helper to create higher fidelity transition names.
 * <p>
 * This can be used by shared element transitions to understand what a particular element actually corresponds to. For example, a listing card may use
 * listing|1337|photo|0 which would have the exact same transition name in a listing marquee and image viewer. However, if the user scrolls to photo 5
 * within the Listing Marquee and then goes back to a page with the listing card, it could be understood that listing|1337|photo|5 can be transitioned
 * to listing|1337|photo|0 with a crossfade.
 */
@AutoValue
public abstract class TransitionName {
     ...
     */
    public static TransitionName create(@NonNull String type, long id, @NonNull String subtype, long subId) {
    ...
    public static String toString(@NonNull String type, long id, @NonNull String subtype) {
         ...
         * Use empty string instead of null of subtype is not applicabale.
         */
    public static String toString(@NonNull String type, long id, @NonNull String subtype, long subId) {
        if (type.indexOf(DELIMETER) != -1) {
            ...
            throw new IllegalArgumentException("Invalid subtype " + subtype + ". Delimeter is " + DELIMETER);
        }
        ...
        switch (parsed.length) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Invalid transition name " + transitionNameString + ". Split into " + parsed.length + ". Should be less than 4.");
        }
    }
    ...
    /**
     * Returns whether at least the type, id, and subtype match.
     * This could be useful when there are 2 elements that represent the same thing (like a listing photo) but they are just different
     * indices. With this knowledge, we can safely maintain the shared element mapping and crossfade them instead.
     */
