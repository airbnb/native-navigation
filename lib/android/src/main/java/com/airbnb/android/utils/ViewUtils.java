...
public final class ViewUtils {
     ...
     * This should be more accurate than getScreenLocationMinusStatusBar since it doesn't need to
     * estimate the status bar height. Try to use this method if possible.
     *
    ...
    public static float getRectOverlapPercentage(Rect rect1, Rect rect2) {
        float intersectionSurfaceArea = Math.max(0, Math.min(rect1.right, rect2.right) - Math.max(rect1.left, rect2.left)) *
    ...
    public static void findTransitionViews(View view, List<Pair<View, String>> transitionViews) {
        ...
        if (!TextUtils.isEmpty(transitionName) && view.getVisibility() == View.VISIBLE && isOnScreen(view)) {
